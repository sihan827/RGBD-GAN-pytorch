...
def downsize_real(x_real, stage, max_stage=17):
    ...
    if stage % 2 == 0:
        ...
        result = x_real
        if scale > 1:
            result = F.avg_pool2d(result, scale, scale, 0)
        ...
        if scale_low > 1:
            result_low = F.interpolate(
                F.avg_pool2d(result_low, scale_low, scale_low, 0),
                size=image_size_high
            )

        if scale_high > 1:
            result_high = F.avg_pool2d(result_high, scale_high, scale_high, 0)
